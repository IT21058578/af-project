...
router.put("/refresh", authorizeRequest([Role.USER]), ...validateSchema({
    refreshToken: { isJWT: true },
    "user-id": { in: ["headers"], isMongoId: true },
}), AuthController.refreshTokens);
router.delete("/logout", authorizeRequest([Role.USER]), ...validateSchema({ "user-id": { in: ["headers"], isMongoId: true } }), AuthController.logoutUser);
...
router.put("/authorize", ...validateSchema({
    email: { isEmail: true, in: ["query"] },
    authorizationToken: { isUUID: true, in: ["query"] },
}), AuthController.authorizeUser);
router.put("/reset-password", ...validateSchema({
    email: { isString: true },
    resetToken: { isUUID: true },
    newPassword: { isString: true },
}), AuthController.resetPassword);
router.put("/forgot-password", ...validateSchema({
    email: { isEmail: true },
}), AuthController.forgotPassword);
router.put("/change-password", authorizeRequest([Role.USER]), ...validateSchema({
    "user-id": { in: ["headers"], isMongoId: true },
